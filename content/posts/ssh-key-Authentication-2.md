...
#### IPtime 공유기를 사용하는 경우
고맙게도 IPtime 제조사는 공유기가 사용할 수 있는 Domain Name과 DDNS서비스를 무료로 제공한다. [IPtime 설정페이지](http://192.168.0.1)로 접속해 로그인한다음 다음의 절차를 따르면 된다.
...
4번에서 레코드라는 단어가 나오는데, 데이터베이스에서 한 줄의 데이터를 의미하는 단어이다. 이 중 A레코드는 Address레코드라는 의미로 IP주소를 지정하는 일반적인 기능을 위한 레코드를 말한다. [참고링크](https://techopedia.com/definition/5349/dns-record) 이 설정에서 A레코드를 비워놓고 IP주소는 임의의 주소를 넣어 놓자. 그리고 DDNS설정을 켜놓으면 나중에 집 컴퓨터에서 DDNS서버로 갱신요청을 보내 컴퓨터가 현재 사용중인 공용 IP주소로 지속적으로 갱신이 가능하다.  
